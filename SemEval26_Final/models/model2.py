...
class EvasionFeatureExtractor:
    def __init__(self):
        logger.log("Loading Sentence Transformer...", "plain")
    ...
    def extract(self, question, answer):
        ...
        if len(sentences) > 1:
            ...
            features["topic_drift"] = features["first_sent_sim"] - \
                features["last_sent_sim"]
        ...
        features["avg_sent_sim"] = (
            features["first_sent_sim"] + features["mid_sent_sim"] + features["last_sent_sim"]) / 3
        features["sim_variance"] = np.var(
            [features["first_sent_sim"], features["mid_sent_sim"], features["last_sent_sim"]])
        ...
        features["q_is_complex"] = int(len(
            q_words) > 20 or features["q_has_and"] or features["q_has_multiple_questions"])
        ...
        features["is_wh_q"] = int(any(q_lower.startswith(w) for w in [
        ...
        features["starts_yes"] = int(
            first_word in direct_yes or any(w in direct_yes for w in first_3))
        features["starts_no"] = int(first_word in direct_no)
        features["starts_direct"] = int(
            features["starts_yes"] or features["starts_no"])
        features["starts_well"] = int(
            first_word == "well" or a_lower.startswith("well,"))
        features["starts_i_dont_know"] = int(a_lower.startswith(
            "i don't know") or a_lower.startswith("i do not know"))
        ...
        features["first_sent_has_q_noun"] = int(
        ...
        features["uncertainty_modal_count"] = sum(
            1 for m in uncertainty_modals if m in a_words)
        ...
        features["short_sentences"] = sum(
            1 for s in sentences if len(s.split()) < 10)
        ...
        hesitation = ["i mean", "you know", "sort of", "kind of",
        ...
        features["hesitation_count"] = sum(
            1 for h in hesitation if h in a_lower)

        comparatives = ["but", "however", "although", "though",
                        "whereas", "nevertheless", "yet", "still"]
        features["has_contrast"] = int(any(c in a_lower for c in comparatives))
        features["contrast_count"] = sum(
            1 for c in comparatives if c in a_lower)
        ...
        commitment = ["promise", "commit", "guarantee", "ensure",
        ...
        ignorance_phrases = ["i don't know", "i do not know", "i'm not sure", "i am not sure",
                             "not certain", "can't say", "cannot say", "no idea", "not aware", "don't have that information"]
        features["claims_ignorance"] = int(
            any(p in a_lower for p in ignorance_phrases))
        features["ignorance_count"] = sum(
            1 for p in ignorance_phrases if p in a_lower)

        decline_phrases = ["i can't answer", "i cannot answer", "i won't", "i will not", "decline to",
                           "refuse to", "not going to answer", "no comment", "not at liberty", "can't discuss"]
        features["declines"] = int(any(p in a_lower for p in decline_phrases))
        features["decline_count"] = sum(
            1 for p in decline_phrases if p in a_lower)

        deflection_phrases = ["you should ask", "that's a question for", "talk to", "ask them",
                              "they would know", "not my area", "someone else", "that's for", "you'd have to ask"]
        features["deflects"] = int(
            any(p in a_lower for p in deflection_phrases))
        features["deflection_count"] = sum(
            1 for p in deflection_phrases if p in a_lower)

        clarify_phrases = ["what do you mean", "could you clarify", "can you explain",
                           "i'm not sure what you", "which one", "are you asking about", "do you mean", "clarify that"]
        features["asks_clarification"] = int(
            any(p in a_lower for p in clarify_phrases))
        features["clarification_count"] = sum(
            1 for p in clarify_phrases if p in a_lower)

        implicit_markers = ["implies", "suggests", "indicates", "means", "essentially",
                            "basically", "in other words", "what i'm saying", "the point is"]
        features["has_implicit_markers"] = int(
            any(m in a_lower for m in implicit_markers))
        features["implicit_count"] = sum(
            1 for m in implicit_markers if m in a_lower)

        features["has_conditional"] = int(
            any(c in a_lower for c in ["if ", "unless ", "whether ", "depending on"]))
        features["conditional_count"] = sum(
            1 for c in ["if ", "unless ", "whether "] if c in a_lower)

        negations = ["not", "no", "never", "nothing", "nobody", "don't",
        ...
        pivot_phrases = ["let me", "the fact is", "the truth is", "what matters", "first of all",
                         "look,", "the real question", "the important thing", "what's important"]
        ...
        causal = ["because", "since", "therefore", "thus",
                  "hence", "as a result", "due to", "reason"]
        ...
        vague_words = ["something", "things", "stuff", "everything",
                       "anything", "people", "everyone", "somebody", "whatever"]
        ...
        features["ignorance_score"] = features["claims_ignorance"] * \
            10 + features["ignorance_count"] * 5
        features["decline_score"] = features["declines"] * \
            10 + features["decline_count"] * 5
        features["deflection_score"] = features["deflects"] * \
            10 + features["deflection_count"] * 5
        features["clarification_score"] = features["asks_clarification"] * \
            10 + features["clarification_count"] * 5

        return features


def train():
    logger.log("TASK 2: EVASION-LEVEL CLASSIFICATION (9 CLASSES)", "announce")

    train_path = os.path.join(DATASET_PATH, "task2_train.parquet")
    if not os.path.exists(train_path):
        logger.log(f"Training data not found at {train_path}", "error")
        logger.log(
            "Please place task2_train.parquet in the dataset folder", "warning")
        return
    ...
    logger.log(f"Total features: {len(feature_names)}", "success")
    ...
    logger.log(f"Train: {len(X_train)}, Test: {len(X_test)}", "plain")

    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_test_scaled = scaler.transform(X_test)

    logger.log("OPTUNA HYPERPARAMETER TUNING", "announce")
    ...
    def objective_lgb(trial):
        ...
        model = lgb.LGBMClassifier(
            **params, class_weight='balanced', random_state=42, verbose=-1, n_jobs=-1)
        scores = cross_val_score(
            model, X_train_scaled, y_train, cv=3, scoring='f1_macro', n_jobs=-1)
        return scores.mean()

    logger.log("Tuning LightGBM...", "plain")
    ...
    logger.log(f"Best LightGBM CV F1: {study_lgb.best_value:.4f}", "success")
    ...
    def objective_xgb(trial):
        ...
        model = xgb.XGBClassifier(
            **params, random_state=42, n_jobs=-1, eval_metric='mlogloss')
        scores = cross_val_score(
            model, X_train_scaled, y_train, cv=3, scoring='f1_macro', n_jobs=-1)
        return scores.mean()

    logger.log("Tuning XGBoost...", "plain")
    study_xgb = optuna.create_study(
        direction='maximize', sampler=TPESampler(seed=42))
    study_xgb.optimize(objective_xgb, n_trials=30, show_progress_bar=True)
    best_xgb_params = study_xgb.best_params
    logger.log(f"Best XGBoost CV F1: {study_xgb.best_value:.4f}", "success")

    logger.log("TRAINING MODELS", "announce")

    models = {
        "LightGBM": lgb.LGBMClassifier(**best_lgb_params, class_weight='balanced', random_state=42, verbose=-1, n_jobs=-1),
        "XGBoost": xgb.XGBClassifier(**best_xgb_params, random_state=42, n_jobs=-1, eval_metric='mlogloss'),
    ...
    logger.log(f"Top 4 for ensemble: {top4_names}", "plain")
